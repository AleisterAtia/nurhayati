    ...
    <div
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <input type="email" placeholder="example@gmail.com" name="email" value="{{ old('email') }}" required autofocus
                    class="w-full px-4 py-3 rounded-xl bg-soft border border-gray-300 focus:ring-2 focus:ring-primary focus:outline-none transition">
            </div>
            ...
            {{-- LOGIN BUTTON --}}
            <button type="submit"
                class="w-full py-3 rounded-xl bg-green hover:bg-greenDark text-white font-semibold shadow-lg transition transform hover:-translate-y-0.5">
                Login
            </button>
        </form>
        ...
        <a href="{{ route('google.redirect') }}"
            class="w-full flex items-center justify-center gap-3 py-3 rounded-xl border bg-white hover:bg-gray-50 shadow transition">
            <i class="fab fa-google text-google text-xl"></i>
            <span class="font-semibold text-secondary">Login dengan Google</span>
        </a>
